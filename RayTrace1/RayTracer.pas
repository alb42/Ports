...
uses Classes, Graphics, Math, {Generics.Collections,} SysUtils, LCLIntf;
...
type
  ...
  Vector = record
    ...
    class operator Multiply(k: FloatVal; const b: Vector): Vector;

    constructor Create(x, y, z: FloatVal);

    function dot(const v: Vector): FloatVal;
    function norm(): Vector;
    function cross(const v: Vector): Vector;
    function mag(): FloatVal;
  end;

  RGBColor = record
    b, g, r, a: Byte end;

    Color = record r, g, b: FloatVal;
    ...
    constructor Create(r, g, b: FloatVal);

    class operator Add(const a, b: Color): Color;
    class operator Multiply(k: FloatVal; const b: Color): Color;
    class operator Multiply(const a, b: Color): Color;

    class function Clamp(v: FloatVal): Byte; static;
  ...
  Intersection = record
    ...
    Ray: Ray;
    dist: FloatVal;
    constructor Create(Thing: Thing; const Ray: Ray; dist: FloatVal);
  ...
  public
    ...
    function reflect(const pos: Vector): FloatVal; virtual; abstract;
    function roughness: FloatVal; virtual; abstract;
  end;
  ...
  public
    ...
    function reflect(const pos: Vector): FloatVal; override;
    function roughness: FloatVal; override;
  end;
  ...
  public
    ...
    function testRay(const Ray: Ray): FloatVal;
  ...
  public
    ...
    constructor Create(center: Vector; radius: FloatVal; Surface: Surface);
  end;
  ...
  public
    ...
    function normal(const pos: Vector): Vector; override;
    function Surface: Surface; override;
    constructor Create(norm: Vector; offset: FloatVal; Surface: Surface);
  end;

implementation

{ Vector }
constructor Vector.Create(x, y, z: FloatVal);
begin
  self.x := x;
  self.y := y;
  self.z := z;
...
function Vector.dot(const v: Vector): FloatVal;
begin
  Result := (x * v.x) + (y * v.y) + (z * v.z);
end;

function Vector.mag: FloatVal;
begin
  Result := Sqrt(x * x + y * y + z * z);
end;

class operator Vector.Multiply(k: FloatVal; const b: Vector): Vector;
begin
  Result.x := k * b.x;
  Result.y := k * b.y;
  Result.z := k * b.z;
...
class operator Color.Multiply(k: FloatVal; const b: Color): Color;
begin
  Result.r := k * b.r;
  Result.g := k * b.g;
  Result.b := k * b.b;
...
class function Color.Clamp(v: FloatVal): Byte;
begin
  if v > 255 then
    Result := 255
  else
...
constructor Color.Create(r, g, b: FloatVal);
begin
  self.r := r;
  self.g := g;
  self.b := b;
...
constructor Intersection.Create(Thing: Thing; const Ray: Ray; dist: FloatVal);
begin
  self.Thing := Thing;
  self.Ray := Ray;
  self.dist := dist;
...
{ Sphere }
constructor Sphere.Create(center: Vector; radius: FloatVal; Surface: Surface);
begin
...
function ShinySurface.reflect(const pos: Vector): FloatVal;
begin
  Result := 0.7;
end;

function ShinySurface.roughness: FloatVal;
begin
  Result := 250;
end;
...
function CheckerboardSurface.reflect(const pos: Vector): FloatVal;
begin
  if ((Floor(pos.z) + Floor(pos.x)) mod 2) <> 0 then
    Result := 0.1
  else
...
function CheckerboardSurface.roughness: FloatVal;
begin
  Result := 150.0;
end;
...
constructor Plane.Create(norm: Vector; offset: FloatVal; Surface: Surface);
begin
  self.norm := norm;
  self.offset := offset;
  self._surface := Surface;
...
begin
  ...
  begin
    ...
    begin
      ...
      //img.Canvas.Pixels[x,y] := RGB(c.r, c.g, c.b);
...
function RayTracerEngine.testRay(const Ray: Ray): FloatVal;
var
  isect: Intersection;
begin
